...
Copy the contents of config/server-template.json to a new file in the same directory named config/server.json, and replace the value in the field marked INSERT HERE with your TMDB api key. **config/server.json is explicitly gitignored and should not be committed.**
